...
class SearchController extends Controller {
    ...
    public function search(Request $request){
        ...
        if(auth()->user()){
            $output =  $this->fetch('/search','POST',json_encode($array));
        }else{
            $output =  $this->fetchNotAuth('/search/open','POST',json_encode($array));
        }
        ...
        return view('search')->with(['limits'=>$limits,'services'=>$services,'companyList'=>$companyList,'experienceSkillList'=>$experienceSkillList,'designationList'=>$designationList,'jobLocationType'=>$jobLocationType]);
    ...
    public function fetchNotAuth($endPoint,$method,$body){
    ...
    public function searchFilter(Request $request){
        ...
        // dd($data);
        if(isset($data['company__name']['values'])){

            $array['filters']->company__name = (object)$data['company__name'];

        }

        if(isset($request['designation__name']['values'])){

            $array['filters']->designation__name = (object)$data['designation__name'];

        }


        if(isset($request['job_location_type']['values'])){

            $array['filters']->job_location_type =  (object)$data['job_location_type'];
        ...
        if(auth()->user()){
            // dd(body($array));
            $output = fetch('POST','/search',body($array));
            // dd($output);
            // $output =  $this->fetch('/search','POST',json_encode($array));
        }else{
            $output =  $this->fetchNotAuth('/search/open','POST',json_encode($array));            
